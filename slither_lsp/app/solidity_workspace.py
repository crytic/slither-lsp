...
class SolidityWorkspace:
    ...
    def main_loop(self) -> None:
        ...
        while not self._shutdown and not self.app.server.context.shutdown:
            if self._analysis_pending and self.compilation_targets_enabled:
            ...
            with self.compilation_targets_lock:
                with self.analyses_lock:
                    ...
                    for compilation_target in self.compilation_targets:
                        ...
                        # Report our last result to the client
                        report_progress_params_results: List[AnalysisResultProgress] = []
                        for i in range(0, len(self.compilation_targets)):
                            if i < len(self.analyses):
                                report_progress_params_results.append(
                                    AnalysisResultProgress(
                                        succeeded=self.analyses[i].succeeded,
                                        compilation_target=self.analyses[i].compilation_target,
                                        error=None if self.analyses[i].error is None else str(self.analyses[i].error)
                                    )
                                )

...
from slither_lsp.app.types.compilation_structures import CompilationTarget, CompilationTargetStandardJson, \
...
class SolidityWorkspace:
    ...
    def __init__(self, app):
        ...
        # Define variables which indicate if changes were made and re-analysis is queued.
        self._analysis_last_change_time: float = 0
        ...
        self._compilation_targets_fixed = False
    ...
    def main_loop(self) -> None:
        ...
        :return: None
        """
        # Discover solidity files in our workspace/open documents. We don't compile now as we want to save time
        # and register for file events as soon as possible.
        self.refresh_workspace(rescan_targets=True, compile_targets=False)
        ...
        while not self._shutdown and not self.app.server.context.shutdown:
            if self._analysis_pending:
                if time.time() - self._analysis_last_change_time > self._FILE_CHANGE_ANALYSIS_DELAY_SECONDS:
                    ...
                    self.refresh_workspace(rescan_targets=False, compile_targets=True)
    ...
    def _queue_reanalysis(self, force: bool = False) -> None:
        """
        Queues all compilation targets for reanalysis.
        :param force: If True, indicates the last analysis time will be set to zero so a reanalysis will be triggered
        in the next polling interval. Otherwise it will be recompiled at the next polling round when the delay time has
        been met.
        :return: None
        """
    ...
    def _on_server_initialized(self, params: InitializeParams, result: InitializeResult) -> None:
        ...
        self.workspace_folders = {
            ...
            for workspace_folder in self._init_params.workspace_folders
        }
    ...
    def set_compilation_targets_fixed(self, targets: List[CompilationTarget]) -> None:
        """
        Sets the compilation targets as fixed, user-supplied targets instead of autogenerating.
        :param targets: The list of compilation targets to set.
        :return: None
        """
        with self.compilation_targets_lock:
            self._compilation_targets_fixed = True
            self.compilation_targets = targets
            self._queue_reanalysis()

    def set_compilation_targets_autogenerated(self) -> None:
        """
    ...
    def refresh_workspace(self, rescan_targets: bool = False, compile_targets: bool = True) -> None:
        ...
        with self.solidity_files_lock:
            ...
            with self.compilation_targets_lock:
                if not self._compilation_targets_fixed:
                    self.compilation_targets = self.generate_compilation_targets()

        # Now that the compilation targets have been updated, compile and analyze each target.
        if compile_targets:
            with self.compilation_targets_lock:
    ...
    @staticmethod
    def _compile_target(compilation_settings: CompilationTarget) -> CryticCompile:
        """

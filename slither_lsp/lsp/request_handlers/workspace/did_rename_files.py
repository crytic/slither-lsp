...
class DidRenameFilesHandler(BaseRequestHandler):
    """
    Handler for the 'workspace/didRenameFiles' notification, which is sent from the client to the server when files
    were renamed from within the client.
    References:
        https://microsoft.github.io/language-server-protocol/specifications/specification-3-17/#workspace_didRenameFiles
    """

    method_name = "workspace/didRenameFiles"
    ...
    def _check_capabilities(cls, context: ServerContext) -> None:
        ...
        server_supported: bool = context.server_capabilities.workspace and \
            ...
            context.server_capabilities.workspace.file_operations.did_rename is not None
    ...
    def process(cls, context: ServerContext, params: Any) -> Any:
        """
        Handles a 'workspace/didRenameFiles' notification which is sent from the client to the server when files were

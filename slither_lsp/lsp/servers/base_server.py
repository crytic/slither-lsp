...
from slither_lsp.lsp.request_handlers import registered_handlers
...
# Obtain all core request handlers for the LSP so we have a request method name -> request handler lookup
_CORE_REQUEST_HANDLERS: dict = {
    ch.method_name: ch
...
_HANDLER_POLLING_INTERVAL: float = 0.1
_RESPONSE_POLLING_INTERVAL: float = 0.1
...
class BaseServer:
    ...
    def __init__(self, server_config: ServerConfig):
        ...
        # Create our request handler lookup from our core request handler lookup and add additional handlers provided
        # through the server configuration.
        self._request_handlers = _CORE_REQUEST_HANDLERS.copy()
        if self.config.additional_request_handlers is not None:
            for additional_handler in self.config.additional_request_handlers:
                if not issubclass(additional_handler, BaseRequestHandler):
                    raise ValueError(
                        f"Server configuration provided an additional request handler which was not the correct type."
                    )
                self._request_handlers[additional_handler.method_name] = additional_handler

    def _main_loop(self, read_file_handle: IO, write_file_handle: IO):
        """
        The main entry point for the server, which begins accepting and processing request_handlers
        on the given IO.
        :return: None
        """
        ...
        if self.config.initial_server_capabilities is not None:
            server_capabilities = self.config.initial_server_capabilities.clone()
        ...
        while True:
            try:
                ...
                result = self.io.read()
                if result is None:
                    sleep(_HANDLER_POLLING_INTERVAL)
                    continue
    ...
    def _handle_request_or_notification(self, message: Any) -> None:
        """
        The dispatcher for a received request or notification. It determines which request handler to call and unpacks
        arguments.
        :param message: The deserialized Language Server Protocol message received over JSON-RPC.
        :return: None
        """
        ...
        try:
            ...
            # Fetch the relevant request handler. If we don't have one, raise an error.
            command_handler: Optional[Type[BaseRequestHandler]] = self._request_handlers.get(method_name)
            if command_handler is None:
                raise LSPError(
                    LSPErrorCode.MethodNotFound,
                    f"A request handler does not exist for method '{method_name}'",
                    None,
                )

            # Execute the relevant request handler and get the result.
            try:
    ...
    def send_request_message(self, method_name: str, params: Any) -> Any:
        ...
        while request_id not in self._pending_response_queue:
            sleep(_RESPONSE_POLLING_INTERVAL)
